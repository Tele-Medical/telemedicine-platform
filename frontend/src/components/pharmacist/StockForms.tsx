...
const StockForms: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mt-6 max-w-2xl">
      ...
      <div className="p-8">
        {status === 'success' ? (
          <div className="py-8 text-center text-green-600 bg-green-50 rounded-lg border border-green-200 animate-fadeIn shadow-sm">
            ...
            <p className="font-bold text-lg text-green-900">Stock Successfully Updated!</p>
            <p className="text-sm text-green-700 mt-1">The inventory ledger has been refreshed.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6 animate-fadeIn">
            <div className="bg-blue-50 border border-blue-100 p-4 rounded-md">
              <p className="text-sm text-blue-800 font-medium">
                {activeTab === 'intake' 
                  ? 'Record new medicine shipments arriving at the facility.' 
                  : 'Adjust existing stock levels due to loss, damage, or audit corrections.'}
              </p>
            </div>
            
            <div>
              <label htmlFor="medicineName" className="block text-sm font-bold text-gray-700 mb-2">Medicine Name</label>
              <input
                id="medicineName"
                type="text"
            ...
            <div>
              <label htmlFor="quantity" className="block text-sm font-bold text-gray-700 mb-2">
                {activeTab === 'intake' ? 'Quantity Received' : 'Adjustment Amount'}
              </label>
              <input
                id="quantity"
                type="number"
                ...
                className="block w-full rounded-md border border-gray-300 px-4 py-3 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 text-gray-900"
                placeholder={activeTab === 'intake' ? "e.g. 500" : "e.g. -10"}
              />
              {activeTab === 'adjustment' && <p className="text-xs text-gray-500 mt-2 font-medium bg-gray-50 inline-block px-2 py-1 rounded">Use negative numbers to reduce stock.</p>}
            </div>
            ...
            <div className="pt-4 border-t border-gray-100">
              ...
              >
                ...
                ) : activeTab === 'intake' ? 'Submit Intake' : 'Submit Adjustment'}
              </button>
            </div>
          </form>
        )}
