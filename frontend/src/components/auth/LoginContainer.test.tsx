...
describe('LoginContainer Component', () => {
  it('renders tab switcher and defaults to Patient Login', () => {
    ...
    expect(screen.getByRole('tab', { name: /auth.patient_portal/i })).toBeInTheDocument();
    expect(screen.getByRole('tab', { name: /auth.staff_portal/i })).toBeInTheDocument();
  ...
  it('switches to Staff Login when tab is clicked and back to Patient Login', () => {
    ...
    const patientTab = screen.getByRole('tab', { name: /auth.patient_portal/i });
    const staffTab = screen.getByRole('tab', { name: /auth.staff_portal/i });
  ...
  it('passes onLogin callback correctly to sub-components', () => {
    ...
    fireEvent.click(screen.getByRole('tab', { name: /auth.staff_portal/i }));
