...
const RecentRecordsList: React.FC<RecentRecordsListProps> = ({ isDemo = false }) => {
  ...
  return (
    <div className="mt-6 mb-8">
      <div className="flex justify-between items-center mb-3">
        <h2 className="text-lg font-semibold text-text-primary">{t('clinical.recent_records')}</h2>
        <button className="text-sm text-primary font-medium hover:underline">{t('clinical.view_all')}</button>
      </div>
