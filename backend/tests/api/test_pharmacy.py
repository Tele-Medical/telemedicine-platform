...
def test_create_prescription(client: TestClient, db_session: Session):
    ...
    medicine = MedicineCatalog(
        id=medicine_id, name="Paracetamol 500mg", type="tablet", code="PARA500"
    )
...
def test_inventory_stock_intake(client: TestClient, db_session: Session):
    ...
    movements = (
        ...
        .filter_by(pharmacy_id=pharmacy.id, movement_type="intake")
        .all()
    )
...
def test_inventory_stock_adjustment(client: TestClient, db_session: Session):
    ...
    batch = StockBatch(
        ...
        expiry_date=datetime(2025, 1, 1).date(),
...
def test_pharmacy_availability(client: TestClient, db_session: Session):
    ...
    response = client.get(
        f"/api/v1/pharmacies/availability?medicine_id={medicine.id}", headers=headers
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
def test_fulfillment_workflow(client: TestClient, db_session: Session):
    ...
    item = PrescriptionItem(
        prescription_id=rx.id, medicine_id=medicine.id, dosage="1x daily", quantity_prescribed=10
    )
    db_session.add(item)
    db_session.commit()

    # 1. Accept Fulfillment
    accept_payload = {"pharmacy_id": str(pharmacy.id)}
    response_accept = client.post(
        f"/api/v1/fulfillments/prescription/{rx.id}/accept", json=accept_payload, headers=headers
    )
    ...
    response_dispense = client.post(
        f"/api/v1/fulfillments/{fulfillment_id}/dispense", json=dispense_payload, headers=headers
    )
