...
def test_grant_and_revoke_consent(client: TestClient, db_session: Session):
    ...
    response = client.post(f"/api/v1/patients/{patient.id}/consents", json=grant_payload, headers=headers)
    ...
    response2 = client.patch(f"/api/v1/patients/{patient.id}/consents/{consent_id}", json=revoke_payload, headers=headers)
...
def test_get_active_consents(client: TestClient, db_session: Session):
    ...
    c1 = Consent(patient_id=patient.id, granted_by_user_id=patient_user.id, purpose="teleconsultation", status="active")
    c2 = Consent(patient_id=patient.id, granted_by_user_id=patient_user.id, purpose="research", status="revoked")
...
def test_audit_event_logging(client: TestClient, db_session: Session):
    ...
    audit_logs = db_session.query(AuditEvent).filter_by(
