...
def test_create_encounter_from_appointment(client: TestClient, db_session: Session):
    ...
    appt_resp = client.post(
        "/api/v1/appointments/", headers=auth_headers(patient_user), json=appt_payload
    )
    ...
    enc_resp = client.post("/api/v1/encounters/", headers=auth_headers(doc_user), json=enc_payload)
...
def test_submit_encounter_summary_success(client: TestClient, db_session: Session):
    ...
    sum_resp = client.post(
        f"/api/v1/encounters/{enc_id}/summary", headers=auth_headers(doc_user), json=summary_payload
    )
