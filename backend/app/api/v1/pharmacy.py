...
from app.schemas.pharmacy import (
    PrescriptionCreate, PrescriptionResponse, PharmacyAvailabilityResponse,
    StockIntakeRequest, StockAdjustmentRequest, FulfillmentAcceptRequest,
    FulfillmentDispenseRequest, FulfillmentResponse
)
...
def create_prescription(request: PrescriptionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_prescription(prescription_id: uuid.UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_pharmacy_availability(medicine_id: uuid.UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def record_stock_intake(request: StockIntakeRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def record_stock_adjustment(request: StockAdjustmentRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.default_role not in ["admin", "pharmacist"]:
        raise HTTPException(status_code=403, detail="Not authorized")
    PharmacyService.record_stock_adjustment(db, request, current_user)
    return {"status": "success"}

@router.post("/fulfillments/prescription/{prescription_id}/accept", response_model=FulfillmentResponse, status_code=status.HTTP_201_CREATED)
def accept_fulfillment(prescription_id: uuid.UUID, request: FulfillmentAcceptRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.default_role not in ["admin", "pharmacist"]:
        raise HTTPException(status_code=403, detail="Not authorized")
...
def dispense_fulfillment(fulfillment_id: uuid.UUID, request: FulfillmentDispenseRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.default_role not in ["admin", "pharmacist"]:
        raise HTTPException(status_code=403, detail="Not authorized")
