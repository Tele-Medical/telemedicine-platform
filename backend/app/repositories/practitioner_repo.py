...
def get_by_id(db: Session, practitioner_id: UUID) -> Optional[Practitioner]:
    return (
        ...
        .filter(Practitioner.id == practitioner_id, Practitioner.is_active)
        .first()
    )
...
def get_by_user_id(db: Session, user_id: UUID) -> Optional[Practitioner]:
    return (
        db.query(Practitioner)
        .filter(Practitioner.user_id == user_id, Practitioner.is_active)
        .first()
    )


def list_all(
    db: Session, specialty: Optional[str] = None, skip: int = 0, limit: int = 100
) -> List[Practitioner]:
    query = db.query(Practitioner).filter(Practitioner.is_active)
    if specialty:
        query = query.filter(Practitioner.specialty_category == specialty)
    return query.order_by(Practitioner.id.asc()).offset(skip).limit(limit).all()
