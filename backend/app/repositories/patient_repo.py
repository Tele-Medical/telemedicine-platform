...
def create(db: Session, obj_in: PatientCreate, created_by_id: Optional[UUID] = None) -> Patient:
    ...
    """
    db_obj = Patient(
        **obj_in.model_dump(), created_by_user_id=created_by_id, updated_by_user_id=created_by_id
    )
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
...
def update(
    db: Session, db_obj: Patient, obj_in: PatientUpdate, updated_by_id: Optional[UUID] = None
) -> Patient:
    """
