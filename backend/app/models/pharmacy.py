...
class StockBatch(Base):
    ...
    pharmacy_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("pharmacies.id"), index=True
    )
    medicine_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("medicine_catalog.id"), index=True
    )
...
class StockMovement(Base):
    ...
    batch_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("stock_batches.id"), index=True
    )
    pharmacy_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("pharmacies.id"), index=True
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "movement_type IN ('intake', 'adjustment', 'dispense')",
...
class Fulfillment(Base):
    ...
    items: Mapped[list["FulfillmentItem"]] = relationship(
        "FulfillmentItem", back_populates="fulfillment", cascade="all, delete-orphan"
    )
...
class FulfillmentItem(Base):
    ...
    fulfillment_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("fulfillments.id"), index=True
    )
    prescription_item_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("prescription_items.id")
    )
