    ...
    try:
        ...
        for phone in phone_variants:
            ...
            for patient in patients:
                ...
                # Delete observations
                obs_deleted = session.query(Observation).filter(Observation.patient_id == patient_id).delete(synchronize_session=False)
                print(f"  Deleted {obs_deleted} Observations")
                
                # Delete medication requests
                med_req_deleted = session.query(MedicationRequest).filter(MedicationRequest.patient_id == patient_id).delete(synchronize_session=False)
                print(f"  Deleted {med_req_deleted} MedicationRequests")

                # Delete encounters
                enc_deleted = session.query(Encounter).filter(Encounter.patient_id == patient_id).delete(synchronize_session=False)
                print(f"  Deleted {enc_deleted} Encounters")

                # Delete appointments
                app_deleted = session.query(Appointment).filter(Appointment.patient_id == patient_id).delete(synchronize_session=False)
                print(f"  Deleted {app_deleted} Appointments")

                # Delete patient identifiers
                ident_deleted = session.query(PatientIdentifier).filter(PatientIdentifier.patient_id == patient_id).delete(synchronize_session=False)
            ...
            for user in users:
                ...
                # Delete sessions
                sess_deleted = session.query(Session).filter(Session.user_id == user_id).delete(synchronize_session=False)
                print(f"  Deleted {sess_deleted} Sessions")

                # Delete device registrations
                dev_deleted = session.query(DeviceRegistration).filter(DeviceRegistration.user_id == user_id).delete(synchronize_session=False)
                print(f"  Deleted {dev_deleted} DeviceRegistrations")

                # Delete OTP challenges
                otp_deleted = session.query(OTPChallenge).filter(OTPChallenge.user_id == user_id).delete(synchronize_session=False)
                print(f"  Deleted {otp_deleted} OTPChallenges")

                # Delete user roles mapping
                roles_deleted = session.query(UserRole).filter(UserRole.user_id == user_id).delete(synchronize_session=False)
            ...
            otp_specific_deleted = session.query(OTPChallenge).filter(OTPChallenge.phone == phone).delete(synchronize_session=False)
